...
class CleanImagesFB():
    ...
    def __init__(self, path: str='data/images', target_folder: str='data/cleaned_images') -> None:
        self.path = path
    ...
    def clean_images(self, size: int=64) -> None:
        ...
        for image in self.images:
            ...
            if str(image.split("/")[-1]) not in list_of_cleaned_images:
                print(f'{image.split("/")[-1]} not in cleaned_images')
                ...
                #save Images
                print(f'Saving cleaned image: cleaned_images/{image_name}')
                black_img.save(f'data/cleaned_images/{image_name}')
            else:
